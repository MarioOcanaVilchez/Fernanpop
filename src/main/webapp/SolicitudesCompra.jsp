...
<head>
    <title>Title</title>
    <link rel="stylesheet" type="text/css" href="CSS/SolicitudesCompra.css?v=5">
...
<ul>
    ...
    <li id="seleccionado"><a href="SolicitudesCompra.jsp">Solicitudes de compra</a></li>
    <li><a href="SolicitudesVenta.jsp">Solicitudes de venta</a></li>
</ul>
<%
    if (gestionAPP.getUsuario() != null) {
        ArrayList<Trato> solicitudesCompra = gestionAPP.getSolicitudesCompra();
        if (solicitudesCompra == null || solicitudesCompra.isEmpty()) out.print("<p>No has solicitado la compra de ningún producto</p>");
        else {
            for (Trato t : solicitudesCompra) {
                out.print("<div class=\"solicitudCompra\">");
                ...
                out.print("<p>" + t.getProducto().getTitulo() + "</p>" +
                        "<p>de " + t.getEmailOtroUser() + " por ");
                if (t.getPrecio() % 1 == 0) out.print((int) t.getPrecio());
                else out.print(t.getPrecio());
                out.print(" €</p>");
                out.print("<button onclick=\"window.location.href='EliminaSolicitudCompra.jsp?id=" + t.getId() + "'\"></button>");
                out.print("</div>");
            }
        }
    }
