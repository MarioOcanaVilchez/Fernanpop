...
<head>
    <%
        session.setAttribute("idProducto",Long.parseLong(request.getParameter("id")));
        session.setAttribute("accion","actualizaProducto");
        GestionAPP gestionAPP = (GestionAPP) session.getAttribute("controller");
        Producto producto = gestionAPP.buscarProductoId(Long.parseLong(request.getParameter("id")));
    %>
    <title><%="Editando " + producto.getTitulo()%></title>
    <link rel="stylesheet" type="text/css" href="CSS/EditaProducto.css?v=2">
...
<form method="post" action="${pageContext.request.contextPath}/SubirImagen" enctype="multipart/form-data">
    <%
        if (producto.getNombreImagen() != null){
            out.print("    <img alt=\"imagenProducto\" src=\"" + request.getContextPath()
                    + "/VerImagen?nombreImagen=" + producto.getNombreImagen() + "\">\n");
        } else out.print("<img alt=\"imagen del producto\" src=\"imagenes/Captura%20de%20pantalla%202026-07-09%20164506.png\">");
    %>
    <input type="file" name="imagen">
    <input type="text" name="titulo" placeholder="nombre" value="<%= producto.getTitulo()%>" required>
    <input type="text" name="descripcion" placeholder="Añade una pequeña descripción" value="<%= producto.getDescripcion()%>" required minlength="20">
    <input type="number" name="precio" step="0.01" required placeholder="Precio" value="<%= producto.getPrecio()%>" min="0">
    <select name="estado">
        <%
            if (producto.getEstado().equals("No determinado")) out.print("<option value=\"No determinado\" selected>No determinado</option>");
            else out.print("<option value=\"No determinado\">No determinado</option>");
            if (producto.getEstado().equals("Como nuevo")) out.print("<option value=\"Como nuevo\" selected>Como nuevo</option>");
            else out.print("<option value=\"Como nuevo\">Como nuevo</option>");
            if (producto.getEstado().equals("Poco usado")) out.print("<option value=\"Poco usado\" selected>Poco usado</option>");
            else out.print("<option value=\"Poco usado\">Poco usado</option>");
            if (producto.getEstado().equals("Bien cuidado")) out.print("<option value=\"Bien cuidado\" selected>Bien cuidado</option>");
            else out.print("<option value=\"Bien cuidado\">Bien cuidado</option>");
            if (producto.getEstado().equals("Usado")) out.print("<option value=\"Usado\" selected>Usado</option>");
            else out.print("<option value=\"Usado\">Usado</option>");
            if (producto.getEstado().equals("Deteriorado")) out.print("<option value=\"Deteriorado\" selected>Deteriorado</option>");
            else out.print("<option value=\"Deteriorado\">Deteriorado</option>");
        %>
    </select>
