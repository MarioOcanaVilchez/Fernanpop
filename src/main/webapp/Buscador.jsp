...
<form method="get" action="ActivaBuscador.jsp">
    <input type="text" name="texto" placeholder="Buscar...">
    <label><input type="radio" name="orden" value="aleatorio">aleatorio</label>
    <label><input type="radio" name="orden" value="menorMayor">precio de menor a mayor</label>
    <label><input type="radio" name="orden" value="mayorMenor">precio de mayor a menor</label>
    <div class="rango-precio">
        <label>Precio</label>

        <div class="rango-precio-valores">
            <span id="valorMin">0€</span>
            <span id="valorMax">2000€</span>
        </div>

        <div class="rango-precio-slider">
            <div class="rango-precio-pista"></div>
            <div class="rango-precio-relleno" id="relleno"></div>
            <input type="range" id="sliderMin" min="0" max="2000" step="1" value="0">
            <input type="range" id="sliderMax" min="0" max="2000" step="1" value="2000">
        </div>

        <!-- Estos son los valores reales que se envían al formulario -->
...
<script>
    (function () {
        ...
        const valorMinTxt = document.getElementById('valorMin');
        const valorMaxTxt = document.getElementById('valorMax');
        const precioMinHidden = document.getElementById('precioMin');
        const precioMaxHidden = document.getElementById('precioMax');

        const separacionMinima = 1; // evita que los dos tiradores se crucen del todo

        function actualizar() {
            let min = parseInt(sliderMin.value, 10);
            let max = parseInt(sliderMax.value, 10);

            // Evita que el mínimo se pase del máximo (y viceversa)
            if (min > max - separacionMinima) {
                min = max - separacionMinima;
                if (min < 0){
                    max = separacionMinima;
                    min = 0;
                }
                sliderMax.value = max
                sliderMin.value = min;
            ...
            const rangoTotal = sliderMin.max - sliderMin.min;
            const porcentajeMin = ((min - sliderMin.min) / rangoTotal) * 100;
            const porcentajeMax = ((max - sliderMin.min) / rangoTotal) * 100;

            relleno.style.left = porcentajeMin + '%';
            relleno.style.right = (100 - porcentajeMax) + '%';

            valorMinTxt.textContent = min + '€';
            if (max === 2000) valorMaxTxt.textContent = '+' + max + '€';
            else valorMaxTxt.textContent = max + '€';

            precioMinHidden.value = min;
            precioMaxHidden.value = max;
        }

        sliderMin.addEventListener('input', actualizar);
        sliderMax.addEventListener('input', actualizar);

        actualizar(); // pinta el estado inicial
    })();
</script>
