...
<ul>
    ...
    <li id="seleccionado"><a href="SolicitudesVenta.jsp">Solicitudes de venta</a></li>
</ul>
<%
    if (gestionAPP.getUsuario() != null) {
        ArrayList<Trato> solicitudesVenta = gestionAPP.getVentasPendientes();
        if (solicitudesVenta == null || solicitudesVenta.isEmpty()) out.print("<p>No tienes ninguna solicitud de venta</p>");
        else {
            for (Trato t : solicitudesVenta) {
                out.print("<div class=\"solicitudVenta\">");
                ...
                out.print("<p>" + t.getProducto().getTitulo() + "</p>" +
                        "<p>solicitados por " + t.getEmailOtroUser() + " ofreciendo ");
                if (t.getPrecio() % 1 == 0) out.print((int) t.getPrecio());
                else out.print(t.getPrecio());
                out.print(" €</p>");
                out.print("<button onclick=\"window.location.href='GestionaSolicitudVenta.jsp?id=" + t.getId() + "&opcion=aceptar'\"></button>");
                out.print("<button onclick=\"window.location.href='GestionaSolicitudVenta.jsp?id=" + t.getId() + "&opcion=rechazar'\"></button>");
                out.print("</div>");
            }
        }
    }
