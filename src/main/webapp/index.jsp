...
<body>
    ...
    <%
        //Bloque que o coge el controlador o lo crea y pone las variables de sesión a null
        ...
        } else gestionAPP = (GestionAPP) session.getAttribute("controller");
        session.setAttribute("accion",null);
        session.setAttribute("errorSesion",null);
        session.setAttribute("error",null);
        session.setAttribute("recomendacion",null);
        session.setAttribute("ErrorNumEntrada",null);
        session.setAttribute("oportunidades",null);
        session.setAttribute("producto",null);
        session.setAttribute("id",null);
        session.setAttribute("nombreImagen",null);
        session.setAttribute("titulo",null);
        session.setAttribute("descripcion",null);
        session.setAttribute("precio",null);
        ...
        //Atributos de busqueda orden y textoBuscar pero tambien dependen de la busqueda numPaginas y productos, al buscar algo pagina cambia a 1
        ArrayList<Producto> productos;
        if (session.getAttribute("orden") == null) session.setAttribute("orden","aleatorio");
        if (session.getAttribute("productos") == null){
            productos = gestionAPP.getPaginaProductos(null,null,"aleatorio",0,Integer.MAX_VALUE);
            session.setAttribute("productos",productos);
        } else productos = (ArrayList<Producto>) session.getAttribute("productos");
        if (session.getAttribute("precioMin") == null) session.setAttribute("precioMin",0);
        if (session.getAttribute("precioMax") == null) session.setAttribute("precioMax",Integer.MAX_VALUE);
        //Al cambiar de pagina se deben traer 12 nuevos productos que cumplan los requisitos y no tengan el mismo id que los que ya hay
    %>
    <!-- banner de arriba-->
    <%
    ...
    <%
        ...
        if (session.getAttribute("textoBuscar") == null) out.print("<button id=\"buscador\" onclick=\"window.location.href = 'Buscador.jsp'\">" +
                "    <span>Buscar...</span>" +
                "    <img alt=\"Lupa\" src=\"imagenes/lupa.png\">\n" +
                "</button>");
        else out.print("<button id=\"buscador\" onclick=\"window.location.href = 'Buscador.jsp'\">" +
                "    <span>" + session.getAttribute("textoBuscar") + "</span>" +
                "    <img alt=\"Lupa\" src=\"imagenes/lupa.png\">\n" +
                "</button>");
        if (!productos.isEmpty()){
            for (int i = 0; i < 12; i++) {
                if (productos.size() - 1 >= i + ((Integer) session.getAttribute("pagina") - 1 ) * 12) {
                    int numProducto = i + ((Integer) session.getAttribute("pagina") - 1 ) * 12;
                    out.print("<div class=\"producto\">\n" +
                            "<button onclick=\"window.location.href = 'ProductoComprar.jsp?id=" + productos.get(numProducto).getId() + "'\">");
                    if (productos.get(numProducto).getNombreImagen() == null) out.print("    <img alt=\"imagenProducto\" src=\"imagenes/Captura%20de%20pantalla%202026-07-09%20164506.png\">\n");
                    else out.print("    <img alt=\"imagenProducto\" src=\"" + request.getContextPath()
                            + "/VerImagen?nombreImagen=" + productos.get(numProducto).getNombreImagen() + "\">\n");
                    out.print("<h1>" + productos.get(numProducto).getTitulo() + "</h1>" + "\n" +
                            "<p>" + productos.get(numProducto).getDescripcion() + "<p>" + "\n" +
                            "<h2>");
                    if (productos.get(numProducto).getPrecio() % 1 == 0) out.print((int) productos.get(numProducto).getPrecio());
                    else out.print(productos.get(numProducto).getPrecio());
                    out.print(" €</h2></button></div>");
                } else i = 12;
            }
            out.print("</div>");
            int paginaActual = (Integer) session.getAttribute("pagina");
            int totalPaginas = (Integer) session.getAttribute("numPaginas");
            out.print("<div id=\"paginacion\">");
            out.print("<button" + (paginaActual <= 1 ? " disabled" : "") + " onclick=\"window.location.href = 'CambiaPagina.jsp?cambio=menos'\">" +
                    "<svg class=\"icono-flecha\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2.2\" stroke-linecap=\"round\" stroke-linejoin=\"round\">" +
                    "<path d=\"M15 6l-6 6 6 6\"/>" +
                    "</svg></button>");
            out.print("<span id=\"textoPagina\">Página " + paginaActual + " de " + totalPaginas + "</span>");
            out.print("<button" + (paginaActual >= totalPaginas ? " disabled" : "") + " onclick=\"window.location.href = 'CambiaPagina.jsp?cambio=mas'\">" +
                    "<svg class=\"icono-flecha\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2.2\" stroke-linecap=\"round\" stroke-linejoin=\"round\">" +
                    "<path d=\"M9 6l6 6-6 6\"/>" +
                    "</svg></button>");
            out.print("</div>");
        } else {
            out.print("<p>No hay productos con esas características</p>");
            out.print("</div>");
        }
