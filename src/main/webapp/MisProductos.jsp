...
<%
    ArrayList<Producto> productos = gestionAPP.getProductosUser();
    if (productos == null || productos.isEmpty()) out.print("<p>No ha puesto a la venta ningún producto");
    else {
        out.print("<div class=\"listaProductos\">\n");
        for (Producto p : productos){
            out.print("<div class=\"producto\">\n");

            // Menú de opciones (tres puntos)
            out.print("<div class=\"menuProducto\">\n");
            out.print("    <button class=\"menuProductoBtn\" onclick=\"toggleMenuProducto(this, event)\">\n" +
                    ...
                    "        </svg>\n" +
                    "    </button>\n");
            out.print("    <div class=\"menuProductoDropdown\">\n" +
                    "        <a href=\"EditaProducto.jsp?id=" + p.getId() + "\">Actualizar</a>\n" +
                    "        <a href=\"EliminaProducto.jsp?id=" + p.getId() + "\">Eliminar</a>\n" +
                    "    </div>\n");
            out.print("</div>\n");

            if (p.getNombreImagen() == null) out.print("    <img alt=\"imagenProducto\" src=\"imagenes/Captura%20de%20pantalla%202026-07-09%20164506.png\">\n");
            else out.print("    <img alt=\"imagenProducto\" src=\"" + request.getContextPath()
                    + "/VerImagen?nombreImagen=" + p.getNombreImagen() + "\">\n");
            out.print("<h1>" + p.getTitulo() + "</h1>" + "\n" +
                    "<p>" + p.getDescripcion() + "<p>" + "\n" +
                    "<h2>");
            if (p.getPrecio() % 1 == 0) out.print((int) p.getPrecio());
            else out.print(p.getPrecio());
            out.print(" €</h2></div>");
        }
        out.print("</div>");
    }
...
<script>
    function toggleMenuProducto(btn, event) {
        ...
        document.querySelectorAll('.menuProductoDropdown.activo').forEach(function (el) {
